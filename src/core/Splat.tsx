...
export type SplatProps = {
  ...
  alphaHash?: boolean
  /** Chunk size for lazy loading, prevents choking the worker, default: 25000 (25kb) */
  chunkSize?: number
  ref?: Ref<TargetMesh>
} & Omit<S3.Props<THREE.Mesh>, 'ref'>
...
const SplatMaterial = /* @__PURE__ */ shaderMaterial(
  {
    ...
    focal: 1000.0,
    centerAndScaleTexture: null as unknown as THREE.DataTexture,
    covAndColorTexture: null as unknown as THREE.DataTexture,
  },
  /*glsl*/ `
...
function createWorker(self: Worker & typeof globalThis) {
  ...
  self.onmessage = (
    ...
    if (e.data.method === 'push') {
      ...
      if (matrices !== null) {
        ...
        ;(self as unknown as Worker).postMessage({ indices, key: e.data.key }, [indices.buffer])
      }
    }
  }
}
...
async function load(shared: SharedState) {
  ...
  const context = shared.gl.getContext() as WebGL2RenderingContext
...
function update(camera: THREE.Camera, shared: SharedState, target: TargetMesh, hashed: boolean) {
  ...
  const mat = target.material as THREE.ShaderMaterial & SplatMaterialType
  ;(mat.viewport as THREE.Vector2).x = target.viewport.z
  ;(mat.viewport as THREE.Vector2).y = target.viewport.w
  mat.focal = (target.viewport.w / 2.0) * Math.abs(camera.projectionMatrix.elements[5]!)
...
function connect(shared: SharedState, target: TargetMesh) {
  ...
  function listener(e: MessageEvent<{ key: string; indices: Uint32Array }>) {
...
function pushDataBuffer(shared: SharedState, buffer: ArrayBufferLike, vertexCount: number) {
  ...
  for (let i = 0; i < vertexCount; i++) {
    ...
    // Store scale and transparency to remove splat in sorting process
  ...
  while (remaining > 0) {
    ...
    if (shared.loadedVertexCount % shared.bufferTextureWidth !== 0) {
      width = Math.min(shared.bufferTextureWidth, xoffset + remaining) - xoffset
      height = 1
    } else if (Math.floor(remaining / shared.bufferTextureWidth) > 0) {
      width = shared.bufferTextureWidth
      height = Math.floor(remaining / shared.bufferTextureWidth)
    } else {
      width = remaining % shared.bufferTextureWidth
      height = 1
    }
...
const sharedStateCache = new Map<string, Promise<SharedState>>()
...
function getSharedState(
  ...
  const key = src
  if (sharedStateCache.has(key)) {
    return sharedStateCache.get(key)!
  }
  const loader = new SplatLoader()
  loader.gl = gl
  loader.chunkSize = chunkSize
  const promise = new Promise<SharedState>((resolve, reject) => {
    loader.load(src, resolve, undefined, reject)
  })
  sharedStateCache.set(key, promise)
  return promise
}
