...
export type BoundsApi = {
  ...
  clip(): BoundsApi
  fit(): BoundsApi
  to: (opts: { position: [number, number, number]; target?: [number, number, number] }) => BoundsApi
}

export type BoundsProps = ParentProps<{
  damping?: number
  fit?: boolean
  clip?: boolean
  observe?: boolean
...
const isOrthographic = (def: Camera): def is OrthographicCamera =>
  def && (def as OrthographicCamera).isOrthographicCamera
const isBox3 = (def: any): def is THREE.Box3 => def && (def as THREE.Box3).isBox3

/** Provide controls to Bounds so it can update target/maxDistance after fitting. */
...
export function Bounds(_props: BoundsProps) {
  ...
  const controls = () => useContext(boundsControlsContext)
  ...
  function getSize(): SizeProps {
    ...
    const fitWidthDistance = isOrthographic(store.camera)
      ? maxSize * 4
      : fitHeightDistance / store.camera.aspect
  ...
  const api: BoundsApi = {
    ...
    refresh(object?: THREE.Object3D | THREE.Box3) {
      ...
      if (controls()?.constructor.name === 'OrthographicTrackballControls') {
        ...
        const direction = store.camera.position
          .clone()
          .sub(controls()!.target)
          .normalize()
          .multiplyScalar(distance)
        store.camera.position.copy(controls()!.target.clone().add(direction))
      }
      return this
    },
    clip() {
      const { distance } = getSize()
      const c = controls()
      if (c) c.maxDistance = distance * 10
    ...
    to({ position, target }: { position: [number, number, number]; target?: [number, number, number] }) {
      ...
      goal.focus.copy(target ? new THREE.Vector3(...target) : center)
      if (props.damping) {
    ...
    fit() {
      current.camera.copy(store.camera.position)
      const c = controls()
      if (c) current.focus.copy(c.target)
      const { center, distance } = getSize()
      const direction = center
        .clone()
        .sub(store.camera.position)
        .normalize()
        .multiplyScalar(distance)
      ...
      if (props.damping) {
        ...
        if (c) {
          c.target.copy(goal.focus)
          c.update()
        }
      }
      if (props.onFit) props.onFit(getSize())
      store.requestRender()
      return this
    },
  }
  ...
  useFrame((_, delta) => {
    if (current.animating) {
      ...
      if (c && !equals(current.focus, goal.focus)) return
